...
public class ConvertKeycode
{
    private Dictionary<KeyCode, Keys> switchKeyDictionary;

    public ConvertKeycode()
    {
        switchKeyDictionary = new Dictionary<KeyCode, Keys>()
        {
            //alphabet
            { KeyCode.A, Keys.A },
            { KeyCode.B, Keys.B },
    ...
    public byte GetWindowKeyCoe(KeyCode key)
    {
        Keys keys;
        switchKeyDictionary.TryGetValue(key, out keys);
        
        //windowKey = (int)switchKeyDictionary[key];
        
        return byte.Parse(((int)keys).ToString());
    }
